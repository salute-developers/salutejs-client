...
describe('Проверяем запросы доступов', () => {
    ...
    let getCurrentPosition = (_: (...args: unknown[]) => void, reject: () => void) => {
        reject();
    };

    let server: Server;

    const buildResponseServerAction = ({
        messageId,
        permissions = [],
        status = 'granted',
    }: {
        ...
        server.on('connection', (socket) => {
            ...
            socket.on('message', (data) => {
                ...
                if (message.messageName === 'SERVER_ACTION') {
                    const d = JSON.parse(message.systemMessage?.data || '{}');

                    expect(
                        JSON.parse(message.meta?.current_app || '{}').app_info?.systemName,
                        'в current_app текущий апп',
                    ).to.eq('assistant');
                    expect(d.app_info, 'в systemMessage appInfo из запроса').to.deep.eq(appInfo);

                    switch (phase) {
                        case 0: // в доступе отказано
                            expect(message.meta.location, 'location не заполнен в meta').to.be.undefined;
                            expect(d.server_action, 'Ответ заполнен').to.deep.eq(
                                buildResponseServerAction({
                                    messageId: 1,
                                    permissions: ['geo'],
                        ...
                        default: // доступ предоставлен
                            expect(JSON.parse(message.meta?.location || '{}'), 'координаты совпадают').to.deep.eq({
                                lat: currentPosition.coords.latitude,
                                lon: currentPosition.coords.longitude,
                                accuracy: currentPosition.coords.accuracy,
                            });
                            expect(d.server_action, 'Ответ заполнен').to.deep.eq(
                                buildResponseServerAction({ messageId: 2, permissions: ['geo'] }),
                            );
                            done();
                            break;
                    }
