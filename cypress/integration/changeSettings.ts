...
describe('Проверяем изменение настроек озвучки', () => {
    ...
    it('Стартовый dubbing всегда -1. Если озвучка доступна и на момент её готовности озвучка включена, то отправится 1', (done) => {
        ...
        server.on('connection', (socket) => {
            socket.on('message', (data) => {
                ...
                if (message.initialSettings) {
                    expect(message.initialSettings.settings?.dubbing).eq(-1);
                ...
                if (message.settings) {
                    expect(message.settings?.dubbing).eq(1);
        ...
        server.on('connection', (socket) => {
            ...
            socket.on('message', (data) => {
                ...
                if (message.text) {
                    ...
                    expect(message.text.type).eq(textCount === 1 ? '' : 'application/ssml');
