...
const createRows = (prevReport: BundleReport, nextReport: BundleReport) => {
    const rows: Row[] = [];
...
const createMdSummary = (rowsGroups: RowsGroups = {}, unit: 'KB' = 'KB'): BundleSummary => {
...
const createRowsGroups = (prevReport: BundleReport, nextReport: BundleReport) => {
    const paths = [...new Set([...Object.keys(prevReport), ...Object.keys(nextReport)])];

    const reportsGroups = paths.reduce<Record<string, { prev: BundleReport; next: BundleReport }>>((acc, path) => {
...
export const createSummary = ({
    ...
    prevReport: BundleReport;
    nextReport: BundleReport;
}): BundleSummary => {
